...
class fieldHelperToolbar extends JObject

{
	...
	function customdjfappend($task = '', $alt = '', $cssClass = '', $link='') {
		$bar = & JToolBar::getInstance ( 'toolbar' );
		$title = JText::_ ( $alt );
		if($link==""){
	...
	function excel() {
		...
		// Add an upload button
		//$bar->appendButton ( 'Popup', 'upload', $alt, "index.php?option=com_media&tmpl=component&task=popupUpload&directory=$directory", 640, 520 );
		$bar->appendButton ( 'Link', 'excel', 'Excel', 'index.php?option=com_djfappend&controller=field&format=xls' );
