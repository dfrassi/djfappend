...
class field_detailModelfield_detail extends JModel {
	...
	function _loadData() {
		...
		if (empty ( $this->_data )) {
			$query = 'SELECT h.*, a.title title, ft.name field_type FROM ' . $this->_table_prefix . 'field AS h, #__content as a, #__djfappend_field_type as ft ' . ' WHERE ft.id = h.id_field_type and a.id = h.id_jarticle and h.id = ' . $this->_id;
